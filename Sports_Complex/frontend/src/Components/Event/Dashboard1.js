...
function Dashboard1() {
    ...
    const fetchData = async () => {
      try {
        ...
        const filtered = eventsRes.data.filter(e => {
          ...
          return eventMonth === month && new Date(e.registrationDeadline).getFullYear() === 2025;
        });
        setEvents(filtered);
  ...
  const COLORS = ['#22c55e','#3b82f6','#facc15','#f97316','#a855f7'];
    ...
    <div className="dashboard-container">
      ...
      <div className="dashboard-header">
        ...
        <p className="dashboard-subtitle">
          Comprehensive overview of your sports events, registrations, and participant analytics for 2025
        </p>
      </div>
      ...
      <div className="month-selector">
        ...
        <span className="current-month-indicator">
          Currently viewing: <strong>{monthNamesFull[month-1]} 2025</strong>
        </span>
      </div>
      ...
      <div className="charts-grid">
        <div className="chart-box">
          <h3>📊 Monthly Event Statistics - 2025</h3>
          <p className="chart-description">Events created and participants registered throughout 2025</p>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={monthlyData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
              <XAxis dataKey="name" stroke="#e2e8f0" />
              <YAxis stroke="#e2e8f0" />
              <Tooltip contentStyle={{ backgroundColor: '#1f2937', border: '1px solid #22c55e', borderRadius: '8px', color:'#e2e8f0' }} />
        ...
        <div className="chart-box">
          <h3>🏆 Event Type Distribution - {monthNamesFull[month-1]}</h3>
      ...
      <div className="events-table-container">
        <div className="table-header">
          <h3>📋 Events for {monthNamesFull[month-1]} 2025</h3>
          <p className="table-description">
            Detailed view of all events scheduled for the selected month
          </p>
        </div>
        {events.length===0 ? (
          <div className="no-events-message">
            ...
            <p>There are no events scheduled for {monthNamesFull[month-1]} 2025.</p>
          </div>
        ) : (
                  ...
                  <tr key={ev._id}>
                    <td>
                      <div className="event-title-cell">
                        ...
                        {ev.description && <small>{ev.description.substring(0,50)}...</small>}
                      </div>
                    </td>
                    <td><span className="event-type-badge">{ev.type || 'General'}</span></td>
                    <td>{ev.venue}</td>
                    <td>{new Date(ev.registrationDeadline).toLocaleDateString()}</td>
                    <td>
                      <div className="participants-cell">
                        <span className="participant-count">{ev.participantsCount || 0}/{ev.maxParticipants}</span>
                        <div className="participant-bar">
                          <div className="participant-fill" style={{width: `${((ev.participantsCount||0)/ev.maxParticipants)*100}%`}}></div>
                        </div>
                      </div>
                    </td>
                    <td>
