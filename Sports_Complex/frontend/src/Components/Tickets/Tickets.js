...
const CreateTicketForm = ({
  ...
  // Priority options with colors and descriptions
  const priorities = [
...
export default function Tickets() {
  ...
  // Generate and download a PDF for a ticket (including non-internal comments)
  const downloadTicketPdf = useCallback(async (ticket) => {
    if (!ticket) return;
    try {
      showPopup({ type: 'info', title: 'Preparing PDF', message: 'Generating your ticket PDF…' });

      // Ensure we have the latest comments
      await fetchCommentsForTicket(ticket._id, { force: true });
      const conversation = conversationState[ticket._id] || { comments: [] };
      const comments = Array.isArray(conversation.comments) ? conversation.comments.filter(c => !c.internal) : [];
      ...
      doc.setFontSize(18);
      doc.text(`Ticket: ${ticket.subject || 'Untitled'}`, marginLeft, cursorY);
      cursorY += 24;

      doc.setFontSize(11);
      doc.text(`Ticket ID: ${ticket._id || 'N/A'}`, marginLeft, cursorY);
      cursorY += 16;
      doc.text(`Category: ${ticket.category || 'N/A'}`, marginLeft, cursorY);
      cursorY += 14;
      doc.text(`Priority: ${ticket.priority || 'N/A'}`, marginLeft, cursorY);
      cursorY += 14;
      doc.text(`Status: ${ticket.status || 'N/A'}`, marginLeft, cursorY);
      cursorY += 14;
      doc.text(`Created: ${formatDisplayDate(ticket.createdAt)}`, marginLeft, cursorY);
      cursorY += 14;
      doc.text(`Last updated: ${formatDisplayDate(ticket.updatedAt || ticket.createdAt)}`, marginLeft, cursorY);
      cursorY += 18;
      ...
      const splitDescription = doc.splitTextToSize(ticket.description || '—', 520);
      doc.setFontSize(10);
      doc.text(splitDescription, marginLeft, cursorY);
      cursorY += (splitDescription.length * 12) + 10;
      ...
      if (comments.length > 0) {
        ...
        const rows = comments.map((c) => ([
          ...
          formatUserName(c.author),
          (c.message || '').replace(/\n/g, ' ')
        ]));

        // Use autotable to render comments neatly
        doc.autoTable({
      ...
      // Save PDF
      const filename = `ticket-${ticket._id || 'export'}.pdf`;
      doc.save(filename);

      showPopup({ type: 'success', title: 'PDF ready', message: 'Your ticket PDF has been downloaded.' });
    } catch (error) {
      console.error('Error generating PDF:', error);
      showPopup({ type: 'error', title: 'PDF failed', message: 'Unable to generate PDF. Please try again.' });
    }
  }, [conversationState, fetchCommentsForTicket, showPopup]);
